...
el_blockhead_t *el_get_header(el_blockfoot_t *foot) {
    el_blockhead_t *head = PTR_MINUS_BYTES(foot, sizeof(el_blockhead_t) + foot->size);
    return head;
}
...
el_blockhead_t *el_block_below(el_blockhead_t *block) {
    el_blockhead_t *lower = PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t));
    if ((void *) lower <= (void *) el_ctl.heap_start) {
        return NULL;
    } else {
        lower = PTR_MINUS_BYTES(block, lower->size + EL_BLOCK_OVERHEAD);
        return lower;
    }
}
...
el_blockhead_t *el_find_first_avail(size_t size) {
    ...
    while (ptr != el_ctl.avail->end) {
            // if there's enough space --> return the address
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size) {
    ...
    // holds size of original
    size_t old_size = block->size;

    // sets block to new size
    block->size = new_size; 

    // gets address of new foot
    el_blockfoot_t* new_foot = el_get_footer(block); 

    // sets size attribute of foot to new size
    ...
    // gets to size
    new_block->size = old_size - (new_size + EL_BLOCK_OVERHEAD);

    // sets foot size to the new size while keeping foot
    foot->size = new_block->size; 
...
void el_merge_block_with_above(el_blockhead_t *lower) {
    // will do nothing of lower is NULL or the state is USED
    if (lower == NULL || lower->state == EL_USED) {
        return;
    }

    el_blockhead_t *higher = el_block_above(lower);
    // also does nothing if block above is NULL or USED
    if (higher == NULL || higher->state == EL_USED) {
        return;
    }

    // footer of block above it 
    ...
    // removes higher and lower from available
    el_remove_block(el_ctl.avail, lower);
    el_remove_block(el_ctl.avail, higher);
    // updates size attribute to new size
    lower->size = new_size;
    foot->size = new_size;
    // moves block to the front of the available memory blocks list
...
void el_free(void *ptr) {
    ...
    //   does nothing if available 
    if(head->state == EL_AVAILABLE) {
        return;
    }
    //   otherwise removes it from used and makes available
    el_remove_block(el_ctl.used, head);
    head->state = EL_AVAILABLE;
    //   adds to front of available list
    el_add_block_front(el_ctl.avail, head);
    //   gets above and below blocks and merges with surrounding
